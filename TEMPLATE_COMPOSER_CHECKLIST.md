...
|----------|-------|-----------------|-------------|
| `PORT` | `3000` | No | **Must be an explicit Railway variable, not just a Dockerfile `ENV` default — confirmed the hard way.** Railway's own edge routing needs `PORT` visible at the platform level to know where to send traffic; a bare `EXPOSE 3000`/Dockerfile `ENV PORT=3000` with no matching Railway variable causes every request to hit Railway's own synthetic fallback response (`x-railway-fallback: true`, a `404` or `502` depending on healthcheck state) instead of ever reaching the app — regardless of whether Metabase itself is healthy. |
| `MB_JETTY_PORT` | `3000` | No | Metabase's own actual port-configuration variable (it does not read Railway's generic `PORT` itself) — set alongside `PORT` above so the app definitely binds where Railway expects it. |
...
- **Healthcheck fails forever, `x-railway-fallback: true` on every response (this is the big one — cost real debugging time to find):** if `curl`ing the domain directly returns Railway's own synthetic 404/502 (check for the `x-railway-fallback: true` header) rather than anything from Metabase itself, this means Railway's edge was never routing to the container at all — not a Metabase/database problem, however much the logs look like one. Confirm `PORT` and `MB_JETTY_PORT` are both set as **explicit Railway variables** (not just Dockerfile defaults) to `3000`. This exact bug was chased for a long time as "must be a bad image version" and "must be a stale database" before actually testing with the healthcheck removed and finding the fallback header — a much faster diagnostic path for any future instance of this symptom.
